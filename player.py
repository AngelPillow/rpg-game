...
class Enemy:
    global eclass , eweapons , ehitpoints , earmourclass , edamage , edamagestr , emindmg , emaxdmg
    ...
    def ecustom(self):
        ...
        self.ehp = input("Select your opponent's HitPoint total (Integer)")
        while True:
            try:
                int(self.ehp)
                break
        ...
        self.eweapon = input("Select your opponent's weapon ")
        while True:
            self.eac = input("Select your ArmourClass total , min = 0 , max = 17.5 (can be float)")
            try:
                float(self.eac)
                ...
                print('Invalid input , try again.')
        while True:
            self.edamage = input("Select your opponent's damage type: [1].3d6 [2].2d12 [3].4d4 [i].info [c].Custom")
...
def main():
    global p1 , e1
    start = input('Would you like to see the game intro? [Y] / [Type Anything]')
    if start.upper() == 'Y':
        intro()
    defaultbattle()
    pass
...
def intro():
    ...
    sleep(2)
    print('You may choose to be one of the listed:')
    sleep(2)
    print('Fighter , Wizard or Monk')
    sleep(2)
    ...
    if mymove == 1:
        ...
        if roll >= enemy.eac:
            ...
            if roll == 20:
                print('### Critical Hit ###\n')
                dmg = 2 * player.damage
                print(f'You dealt a total of {dmg} points of damage to your opponent!')
                enemy.ehp -= dmg
            else:
                dmg = player.damage
        ...
        if player.ac >= 15:
            print('You cannot make it harder for your opponent to hit you...')
        else:
            c[1] += 1
            ch = 2
    ...
    if theirmove == 1:
        ...
        if eroll >= player.ac:
            ...
            if eroll == 20:
                ...
                print(f'Your opponent dealt {edmg} points of damage to you!')
                player.hp -= edmg
            else:
                edmg = enemy.edamage
                print(f'Your opponent dealt {edmg} points of damage to you!')
        ...
        if enemy.eac >= 17.5:
            print("Your opponent cannot make it any harder for you to hit")
        else:
            print('It will be now harder to hit your opponent!\n')
            enemy.eac += 0.5
    elif theirmove == 3:
        print("Your opponent's next attacks will be more devastating!\n")
        enemy.edamage += d(1,10)
        enemy.emindmg += 1
        enemy.emaxdmg += 10

def showdown(player,enemy):
    print(f'The battle between you and the opposing {enemy.eclass} have begun!\n')
    while True:
        if player.alive() and enemy.ealive():
            ...
            print('~~~You have died~~~')
            break
        print(f"~~~ Your HP : [{player.hp}] ~~~  VS  ~~~Your opponent's HP : [{enemy.ehp}] ~~~\n")
